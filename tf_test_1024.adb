-- Test of the Threefish cipher for blocks of 1024-bits (Threefish_1024)
...
with PragmARC.Encryption.Threefish_1024;

procedure Tf_Test_1024 is
   use PragmARC.Encryption;

   Key1   : constant Threefish_1024.Block  := (others => 0);
   Tweak1 : constant Threefish_1024.Couple := (others => 0);
   In1    : constant Threefish_1024.Block  := (others => 0);
   Out1   : constant Threefish_1024.Block  := -- f05c3d0a3d05b304 f785ddc7d1e03601 5c8aa76e2f217b06 c6e1544c0bc1a90d
      ...
      Threefish_1024.Block_From_Bytes ( (16#f0#, 16#5c#, 16#3d#, 16#0a#, 16#3d#, 16#05#, 16#b3#, 16#04#,
...
--      16#c4a36c28434a5b9a#, 16#d54331444b1046cf#, 16#df11834830b2a460#, 16#1e39e8dfe1f7ee4f#);
   Key2   : constant Threefish_1024.Block  := -- 1011121314151617 18191a1b1c1d1e1f 2021222324252627 28292a2b2c2d2e2f
      ...
      Threefish_1024.Block_From_Bytes ( (16#10#, 16#11#, 16#12#, 16#13#, 16#14#, 16#15#, 16#16#, 16#17#,
...
--      16#7776757473727170#, 16#7f7e7d7c7b7a7978#, 16#8786858483828180#, 16#8f8e8d8c8b8a8988#);
   Tweak2 : constant Threefish_1024.Couple := -- 000102030405060708090a0b0c0d0e0f
      (Threefish_1024.Word_From_Bytes ( (16#00#, 16#01#, 16#02#, 16#03#, 16#04#, 16#05#, 16#06#, 16#07#) ),
       Threefish_1024.Word_From_Bytes ( (16#08#, 16#09#, 16#0a#, 16#0b#, 16#0c#, 16#0d#, 16#0e#, 16#0f#) ) );
--      (16#0706050403020100#, 16#0f0e0d0c0b0a0908#)
   In2    : constant Threefish_1024.Block  := -- fffefdfcfbfaf9f8 f7f6f5f4f3f2f1f0 efeeedecebeae9e8 e7e6e5e4e3e2e1e0
      ...
      Threefish_1024.Block_From_Bytes ( (16#ff#, 16#fe#, 16#fd#, 16#fc#, 16#fb#, 16#fa#, 16#f9#, 16#f8#,
...
--  16#98999a9b9c9d9e9f#, 16#9091929394959697#, 16#88898a8b8c8d8e8f#, 16#8081828384858687#);
   Out2   : constant Threefish_1024.Block  := -- a6654ddbd73cc3b0 5dd777105aa849bc e49372eaaffc5568 d254771bab85531c
      ...
      Threefish_1024.Block_From_Bytes ( (16#a6#, 16#65#, 16#4d#, 16#db#, 16#d7#, 16#3c#, 16#c3#, 16#b0#,
...
--      16#b316570a15f41333#, 16#74e98a2869f5d50e#, 16#57ce6f9247432bce#, 16#de7cdd77215144de#);
   Key3   : constant Threefish_1024.Block  := (1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768);
   Tweak3 : constant Threefish_1024.Couple := (16, 32);

   function Image is new PragmARC.Images.Modular_Image (Number => Threefish_1024.Word);
   function Image is new PragmARC.Images.Modular_Image (Number => Threefish_1024.Byte);

   procedure Put (Text : in Threefish_1024.Block); -- Outputs the images of the words ot Text on a single line

   procedure Put (Text : in Threefish_1024.Block) is
      -- Empty
   begin -- Put
   ...
   Ks   : Threefish_1024.Key_Schedule_Handle;
   Text : Threefish_1024.Block;

   use type Threefish_1024.Block;
begin -- Tf_Test
   Threefish_1024.Create_Key_Schedule (Key => Key1, Tweak =>Tweak1, Key_Schedule => Ks);
   Ada.Text_IO.Put_Line (Item => "Test case 1: Encryption key = (others => 0), tweak = (others => 0), text = (others => 0)");
   Text := In1;
   Threefish_1024.Encrypt (Key_Schedule => Ks, Text => Text);
   Put (Text => Text);
   Ada.Text_IO.New_Line;
   Ada.Text_IO.Put_Line (Item => "Test passes: " & Boolean'Image (Text = Out1) );
   Threefish_1024.Decrypt (Key_Schedule => Ks, Text => Text);
   Ada.Text_IO.Put_Line (Item => "Decryption Test passes: " & Boolean'Image (Text = in1) );
   Ada.Text_IO.Put (Item => "Decrypts to:");
   Put (Text => Text);
   Ada.Text_IO.New_Line (2);

   Threefish_1024.Create_Key_Schedule (Key => Key2, Tweak =>Tweak2, Key_Schedule => Ks);
   ...
   Threefish_1024.Create_Key_Schedule (Key => Key3, Tweak =>Tweak3, Key_Schedule => Ks);
   ...
   declare
      Ct : constant Threefish_1024.Block_List := -- "ABCDEFGHIJKL"
         Threefish_1024.Encrypt (Ks, (16#41#, 16#42#, 16#43#, 16#44#, 16#45#, 16#46#, 16#47#, 16#48#, 16#49#, 16#50#, 16#51#, 16#52#) );
      Pt : constant Threefish_1024.Byte_List  := Threefish_1024.Decrypt (Ks, Ct);
   begin
