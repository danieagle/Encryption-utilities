-- Test of the Threefish cipher for blocks of 512-bits (Threefish_512)
...
with PragmARC.Encryption.Threefish_512;

procedure Tf_Test_512 is
   use PragmARC.Encryption;

   Key1   : constant Threefish_512.Block  := (others => 0);
   Tweak1 : constant Threefish_512.Couple := (others => 0);
   In1    : constant Threefish_512.Block  := (others => 0);
   Out1   : constant Threefish_512.Block  := -- b1a2bbc6ef6025bc 40eb3822161f36e3 75d1bb0aee3186fb d19e47c5d479947b
                                             -- 7bc2f8586e35f0cf f7e7f03084b0b7b1 f1ab3961a580a3e9 7eb41ea14a6d7bbe
      Threefish_512.Block_From_Bytes ( (16#b1#, 16#a2#, 16#bb#, 16#c6#, 16#ef#, 16#60#, 16#25#, 16#bc#,
...
--      16#cff0356e58f8c27b#, 16#b1b7b08430f0e7f7#, 16#e9a380a56139abf1#, 16#be7b6d4aa11eb47e#);
   Key2   : constant Threefish_512.Block  := -- 1011121314151617 18191a1b1c1d1e1f 2021222324252627 28292a2b2c2d2e2f
                                             -- 3031323334353637 38393a3b3c3d3e3f 4041424344454647 48494a4b4c4d4e4f
      Threefish_512.Block_From_Bytes ( (16#10#, 16#11#, 16#12#, 16#13#, 16#14#, 16#15#, 16#16#, 16#17#,
...
--      16#3736353433323130#, 16#3f3e3d3c3b3a3938#, 16#4746454443424140#, 16#4f4e4d4c4b4a4948#);
   Tweak2 : constant Threefish_512.Couple := -- 000102030405060708090a0b0c0d0e0f
      (Threefish_512.Word_From_Bytes ( (16#00#, 16#01#, 16#02#, 16#03#, 16#04#, 16#05#, 16#06#, 16#07#) ),
       Threefish_512.Word_From_Bytes ( (16#08#, 16#09#, 16#0a#, 16#0b#, 16#0c#, 16#0d#, 16#0e#, 16#0f#) ) );
--      (16#0706050403020100#, 16#0f0e0d0c0b0a0908#)
   In2    : constant Threefish_512.Block  := -- fffefdfcfbfaf9f8 f7f6f5f4f3f2f1f0 efeeedecebeae9e8 e7e6e5e4e3e2e1e0
                                             -- dfdedddcdbdad9d8 d7d6d5d4d3d2d1d0 cfcecdcccbcac9c8 c7c6c5c4c3c2c1c0
      Threefish_512.Block_From_Bytes ( (16#ff#, 16#fe#, 16#fd#, 16#fc#, 16#fb#, 16#fa#, 16#f9#, 16#f8#,
...
--      16#d8d9dadbdcdddedf#, 16#d0d1d2d3d4d5d6d7#, 16#c8c9cacbcccdcecf#, 16#e7e6e5e4e3e2e1e0#);
   Out2   : constant Threefish_512.Block  := -- e304439626d45a2c b401cad8d636249a 6338330eb06d45dd 8b36b90e97254779
                                             -- 272a0a8d99463504 784420ea18c9a725 af11dffea1016234 8927673d5c1caf3d
      Threefish_512.Block_From_Bytes ( (16#e3#, 16#04#, 16#43#, 16#96#, 16#26#, 16#d4#, 16#5a#, 16#2c#,
...
--      16#043546998d0a2a27#, 16#25a7c918ea204478#, 16#346201a1fedf11af#, 16#3daf1c5c3d672789#);
   Key3   : constant Threefish_512.Block  := (1, 2, 4, 8, 16, 32, 64, 128);
   Tweak3 : constant Threefish_512.Couple := (16, 32);

   function Image is new PragmARC.Images.Modular_Image (Number => Threefish_512.Word);
   function Image is new PragmARC.Images.Modular_Image (Number => Threefish_512.Byte);

   procedure Put (Text : in Threefish_512.Block); -- Outputs the images of the words ot Text on a single line

   procedure Put (Text : in Threefish_512.Block) is
      -- Empty
   begin -- Put
   ...
   Ks   : Threefish_512.Key_Schedule_Handle;
   Text : Threefish_512.Block;

   use type Threefish_512.Block;
begin -- Tf_Test
   Threefish_512.Create_Key_Schedule (Key => Key1, Tweak =>Tweak1, Key_Schedule => Ks);
   Ada.Text_IO.Put_Line (Item => "Test case 1: Encryption key = (others => 0), tweak = (others => 0), text = (others => 0)");
   Text := In1;
   Threefish_512.Encrypt (Key_Schedule => Ks, Text => Text);
   Put (Text => Text);
   Ada.Text_IO.New_Line;
   Ada.Text_IO.Put_Line (Item => "Test passes: " & Boolean'Image (Text = Out1) );
   Threefish_512.Decrypt (Key_Schedule => Ks, Text => Text);
   Ada.Text_IO.Put_Line (Item => "Decryption Test passes: " & Boolean'Image (Text = in1) );
   Ada.Text_IO.Put (Item => "Decrypts to:");
   Put (Text => Text);
   Ada.Text_IO.New_Line (2);

   Threefish_512.Create_Key_Schedule (Key => Key2, Tweak =>Tweak2, Key_Schedule => Ks);
   ...
   Threefish_512.Create_Key_Schedule (Key => Key3, Tweak =>Tweak3, Key_Schedule => Ks);
   ...
   declare
      Ct : constant Threefish_512.Block_List := -- "ABCDEFGHIJKL"
         Threefish_512.Encrypt (Ks, (16#41#, 16#42#, 16#43#, 16#44#, 16#45#, 16#46#, 16#47#, 16#48#, 16#49#, 16#50#, 16#51#, 16#52#) );
      Pt : constant Threefish_512.Byte_List  := Threefish_512.Decrypt (Ks, Ct);
   begin
